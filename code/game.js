...
Vector.prototype.plus = function(themOther) {
  return new Vector(this.x + themOther.x, this.y + themOther.y);
};
...
function Athing(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, -3));
...
DOMDisplay.prototype.scrollPlayerIntoView = function() {
  ...
  var middel = puppet.pos.plus(puppet.size.times(0.5))
                 .times(scale);

  if (middel.x < left + margin)
    this.wrap.scrollLeft = middel.x - margin;
  else if (middel.x > right - margin)
    this.wrap.scrollLeft = middel.x + margin - width;
  if (middel.y < top + margin)
    this.wrap.scrollTop = middel.y - margin;
  else if (middel.y > bottom - margin)
    this.wrap.scrollTop = middel.y + margin - height;
};
...
Level.prototype.obstacleAt = function(pos, size) {
  ...
  if (xBegin < 0 || xEnd > this.width || yBegin < 0)
    return "wallFloor";
  if (yEnd > this.height)
    return "spike";

  // Check each grid position starting at yBegin, xBegin
...
Level.prototype.actorAt = function(actor) {
  ...
  for (var i = 0; i < this.actors.length; i++) {
    var themOther = this.actors[i];
    // if the themOther actor isn't the acting actor
    if (themOther != actor &&
        actor.pos.x + actor.size.x > themOther.pos.x &&
        actor.pos.x < themOther.pos.x + themOther.size.x &&
        actor.pos.y + actor.size.y > themOther.pos.y &&
        actor.pos.y < themOther.pos.y + themOther.size.y)
      // check if the boundaries overlap by comparing all sides for
      // overlap and return the themOther actor if found
      return themOther;
  }
};
...
Level.prototype.animate = function(step, keys) {
  ...
  while (step > 0) {
    var playerStep = Math.min(step, maxStep);
    this.actors.forEach(function(actor) {
      // Allow each actor to act on their surroundings
      actor.act(playerStep, this, keys);
    }, this);
...
Puppet.prototype.moveX = function(step, level, keys) {
  ...
  var movment = new Vector(this.speed.x * step, 0);
  // Find out where the puppet character will be in this frame
  var newPos = this.pos.plus(movment);
...
Puppet.prototype.moveY = function(step, level, keys) {
  ...
  this.speed.y += step * maxWeight;;
  var movment = new Vector(0, this.speed.y * step);
  var newPos = this.pos.plus(movment);
  var obstacle = level.obstacleAt(newPos, this.size);
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Puppet loses
  if (type == "spike" && this.status == null || type == "chip" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "athing") {
    this.actors = this.actors.filter(function(themOther) {
      return themOther != actor;
    });
